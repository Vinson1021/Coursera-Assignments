...
%%Compute the forward propagation and each node of the network
a1 = [ones(m,1),X];
z2 = a1*Theta1';
...
J = J+lambda/(2*m)*(sum(sum(Theta1.^2))+sum(sum(Theta2.^2))-sum(Theta1(:,1).^2)-sum(Theta2(:,1).^2));%except the bias node
